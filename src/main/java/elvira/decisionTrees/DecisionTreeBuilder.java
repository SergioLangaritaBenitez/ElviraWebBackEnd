...
public class DecisionTreeBuilder {
	 ...
	 * @return true si est� activada la poda de asimetrias estructurales, false si no.
	 */
	...
	/**
	 * Proxy de funciones supervalor (suma, multiplicaci�n,...)
	 */
	...
	/** Asegura que solo existe un objeto singleton: si no est� creado lo hace y si
	 * ya est� devuelve la referencia a �l
	 * 
	 * @return la �nica instancia creada de este objeto 
	 */
	...
	public static void main(String args[]) throws ParseException, IOException {
		...
		ResourceBundle bundle= ResourceBundle.getBundle("localize/DecisionTrees_sp");
	...
	public AbstractCompositeNode buildDT(IDWithSVNodes diag) throws DTBuildingException {
		...
		// Si est�n activas las podas, podr�a ocurrir que se pode el arbol por completo
		if (tree==null) {
	...
	/** Este m�todo se encarga de a�adir los nodos de utilidad y supervalor al arbol de decision
	 * 
	...
	protected AbstractNode getSuperValueStructure( ArcReversalSV_ID2DT eval, Node node, Configuration configuration) throws DTBuildingException {
		 ...
		 * se necesita almacenar el ID original (quiz� solo sus relaciones)
		 */
	 ...
	 * DONE: Distinguir entre el caso de que haya saltado una constraint -> no eliminar salvo q est� activo
	 * 		'podarChance': quiza sea interesante a�adir un booleano 'podarAsimetriaEstructural'
	...
	protected AbstractNode buildTree( ArcReversalSV_ID2DT eval, Configuration configuration, int depth  ) throws DTBuildingException {
		/* Una vez tratados todos los nodos de azar y decision del arbol, seg�n su orden de eliminaci�n,
		...
		if( depth == eval.getEliminatedVariables().size() ) {
			// A�ade nodos utilidad/sv para cada una de las configuraciones posibles
