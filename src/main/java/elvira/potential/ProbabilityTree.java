...
public class ProbabilityTree {
    ...
    public boolean pruneUtility(double limit,double minG, double maxG,long numberDeleted[]) {
        int i, numberChildren;
        ProbabilityTree ch;
        double max=0,min=0;
        ...
        for(i=0; i < numberChildren; i++){
            ...
            if (ch.label != PROBAB_NODE){
                ...
                long chOldLeaves = ch.leaves;
                childBounded = ch.pruneUtility(limit,minG,maxG,numberDeleted);
                leaves -= (chOldLeaves - ch.leaves);
        ...
        if(bounded == true){
            ...
            // Check the difference between maximun and minimum
  ...
  while (inProportional ) {
   ...
   // In other case get another "first child" valueI and the next, valueJ 
    ...
    if(inProportional){
     ...
     // if there�s at lease one factor then factorise
 ...
 * @return  <code>1</code> if they�re not proportional or one of the values is null, or
 ...
 * or -1 if they�re not proportional with the two previous values
 * compared.
 ...
 *
 * @return -1 if they�re not proportional(alpha has been descarted by the 
